...
PRODUCTS = [  # <----- NO INDENT HERE!
{
      "id": 1,
      "title": "Sony WH-1000XM3 Bluetooth Wireless Over Ear Headphones with Mic (Silver)",
      "image": "https://asset.conrad.com/media10/isa/160267/c1/-/en/1891709_BB_03_FB/image.jpg",
      "price": 773,
    ...
    },
    {
      ...
      "image": "https://i5.walmartimages.com/asr/1f5923d8-94e6-48f8-8301-b03f11c49a9c.0d0f57d0b6998726b61d157ae17eeb3c.jpeg",
      "price": 57,
...
def search():
    ...
    def ok_price(p):
        try:
            if min_price not in (None, '') and p['price'] < float(min_price): return False
            if max_price not in (None, '') and p['price'] > float(max_price): return False
        except ValueError:
            pass  # ignore bad numbers and just don't filter by price
        return True

    if not q and not min_price and not max_price:
        results = PRODUCTS[:]  # nothing typed -> show all
    else:
        results = []
        for p in PRODUCTS:
            hay = " ".join([
            ...
            if (not q or q in hay) and ok_price(p):
                results.append(p)
...
@app.route('/catalog')
def catalog():
    return render_template('front/catalog.html', products=PRODUCTS, module='product')
...
def cart():
    ...
    for pid, qty in cart.items():
        ...
        if prod:
            ...
            cart_items.append({'product': prod, 'qty': qty, 'total': item_total})
            total += item_total
...
#         order_lines = [f"{item['product']['name']} x {item['qty']} = ${item['total']}" for item in cart_items]
#         order_text = "\n".join(order_lines)
#
...
#         email_subject = "Your Order Invoice"
#         email_body = (
...
#     url = f"https://api.telegram.org/bot{8021006374:AAGruM-a23yXeo6gDeN-NoZzV4z0RtPppkk}/sendMessage"
#     data = {
#         'chat_id': 7240810938,
#         'text': message
#     }
...
#
# def send_invoice_email(to, subject, body):
...
def checkout():
    ...
    if request.method == 'POST':
        try:
            ...
            # Prepare order summary
            order_lines = [f"{item['product']['title']} x {item['qty']} = ${item['total']}" for item in cart_items]
            order_text = "\n".join(order_lines)
...
def contact():
    if request.method == 'POST':
        ...
        try:
            owner_to = getattr(Config, 'MAIL_CONTACT_TO', None) or app.config.get('MAIL_USERNAME')
            if not owner_to:
                flash('Email is not configured on the server.', 'error')
                return render_template('front/contact.html')

            subject = f"Contact form: {name}"
            body = f"From: {name} <{email}>\n\nMessage:\n{message}"

            # make sure we have a sender; fall back to MAIL_USERNAME
            sender_addr = app.config.get('MAIL_DEFAULT_SENDER') or app.config.get('MAIL_USERNAME') or owner_to
            ...
            # optional Telegram ping (ignore any failure)
            try:
                send_telegram_notification(f"Contact form\nFrom: {name} <{email}>\n\nMessage:\n{message}")
            except Exception:
                pass
